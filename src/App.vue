            ...
            <div class="w-75 mx-auto">
              ...
              >
                <input
                  type="hidden"
                  name="_token"
                  value="pT5bOFX4FRdpTCJGtPSq3WQ7ejVHH6mIGwXJtHSi"
                />
                <input
                  type="hidden"
                  name="order_id"
                ...
                <input
                  ...
                  name="user_id"
                  id="user_id"
                  :value="GET_ORDER.user_id"
                />

                <div class="form-group">
                  <label for="name" class="fw-600 text-black-50 fs-12"
                    >Name</label
                  >
                  <input class="form-control rounded-" id="name" type="text" />
                </div>
                <div class="form-group">
                  <label for="ccnumber" class="fw-600 text-black-50 fs-12"
                    >Credit Card Number</label
                  >
                  <div class="input-group">
                    <div class="input-group-prepend" id="cardIcons"></div>
                    <input
                      class="form-control card-number rounded-3"
                      id="cardnumber"
                      type="text"
                      placeholder="1242 1234 1234 1234"
                    />
                    <div
                      class="invalid-feedback card-valid-lable"
                      style="display: none"
                    >
                ...
                <div class="form-group">
                  <div class="row">
                    <div class="col-6">
                      <label for="ccyear" class="fw-600 text-black-50 fs-12"
                        >Card Expiration</label
                      >
                      <input
                        class="form-control rounded-3 card-expiry"
                        type="text"
                        placeholder="MM/YY"
                      />
                    </div>
                    <div class="col-6">
                      <label for="cvv" class="fw-600 text-black-50 fs-12"
                        >CVV/CVC</label
                      >
                      <input
                        class="form-control card-cvc rounded-3"
                        id="cvv"
                        type="text"
                        placeholder="123"
                        maxlength="4"
                ...
                <div class="form-group">
                  <button
                    class="btn btn-sm btn-primary form-control rounded-3"
                    type="submit"
                  >
              ...
              <table class="table table-borderless align-middle">
                ...
                <tbody>
                  <tr v-for="o in GET_ORDER.items" :key="o.id">
                    <td>
                      <img
                        :src="o.price.good.photos[0].name"
...
export default {
  ...
  methods: {
    ...
    // sumbit(){
    //   this.stripe_form = {
    //     "user_id" : this.GET_ORDER.user_id,
    //     "order_id" : this.GET_ORDER.order_id,
    //     "stripeToken" : localStorage.getItem('stripe_token')
    //   }
    //   console.log(this.stripe_form)
    //   $http.stripePost('stripe')
    //   .then((res)=>{
    //     console.log(res);
    //   });
    // },
    stripePay() {
      ...
      console.log(this.stripe_form);
      $http.stripePost("vue_stripe", this.stripe_form).then((res) => {
        console.log(res);
      });
    },
    cancelOrder() {
    ...
    getUser() {
      $http.get("customers", localStorage.getItem("user_id")).then((res) => {
    ...
    getCartFromDB() {
      $http
        .getAll("carts?user_id=" + localStorage.getItem("user_id"))
        .then((res) => {
    ...
    getFavFromDB() {
      $http
        .getAll("favorites?user_id=" + localStorage.getItem("user_id"))
        .then((res) => {
          //Add To Cart To Vuex
          let filter_good_not_null = res.data.data.filter(
            (el) => el.good !== null
          );
